...
function StreamerContent() {
            ...
            if (wsRef.current && wsRef.current.readyState === WebSocket.OPEN) {
              wsRef.current.send(JSON.stringify({
                ...
                payload: {
                  ...
                  starAmount: giftAmount,
                  message: giftMsg,
                  totalStars: data.data.totalStars // Số sao lũy kế mới từ DB
                }
              }));
            }
          }
  ...
  const handleEndStream = async () => {
    ...
    try {
      // A. Thông báo qua WebSocket trước để Viewer biết phòng đã đóng
      if (wsRef.current) {
        wsRef.current.send(JSON.stringify({ type: "end-stream" }));
      }

      // B. Gửi yêu cầu PUT tới API `/api/streams` để cập nhật trạng thái phòng thành ENDED
      ...
      <div className={styles.mainContent}>
        ...
        <div className={styles.videoSection}>
          {useCamera ? (
            ...
            <canvas ref={canvasRef} className={styles.virtualCanvas} />
          )}
        ...
        <div className={`${styles.controlBar} premium-card`}>
          <div className={styles.controlLeft}>
            ...
            >
              {useCamera ? "🔌 Tắt Camera (Virtual Mode)" : "📸 Bật Webcam thực tế"}
            </button>
            ...
            <label className={styles.simulatorLabel}>
              ...
              />
              🤖 Bật giả lập người xem & tặng sao tự động (Bots)
            </label>
          </div>
